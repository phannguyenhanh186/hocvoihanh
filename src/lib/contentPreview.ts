...
export function getBankItemPreview(
  ...
  if (kind === "vocabulary") {
    ...
    if (!t) return null;
    return { kind, title: t.title, level: t.level, count: t.questions.length, vocabQuestions: t.questions, audioId: t.audioId };
  }
  if (kind === "listening") {
...
export function getSnapshotPreview(item: HomeworkContentItem): ContentPreviewData {
  switch (item.sourceKind) {
    ...
    case "vocabulary":
      return { kind: "vocabulary", title: item.title, level: item.level, count: item.questions.length, vocabQuestions: item.questions, audioId: item.audioId };
    case "listening":
      return {
        kind: "listening",
        title: item.title,
