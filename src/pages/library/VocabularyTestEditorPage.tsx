...
export default function VocabularyTestEditorPage() {
  ...
  const [audioId, setAudioId] = useState<string | undefined>(test?.audioId);
  ...
  function handleSave() {
    ...
    useAppStore.setState((state) => ({
      vocabularyTests: state.vocabularyTests.map((t) =>
        t.id === test!.id ? { ...t, flashcardSetIds: selectedSetIds, level: level || undefined, audioId } : t
      ),
    }));
  ...
  return (
    <div className="max-w-3xl">
      ...
      <div className="card p-5 mb-6">
        <label className="field-label">Audio bài nghe (tùy chọn)</label>
        <p className="text-xs text-ink/45 mb-2">
          Nếu bài test có phần nghe, tải file MP3 lên — học viên sẽ nghe file này khi làm bài.
        </p>
